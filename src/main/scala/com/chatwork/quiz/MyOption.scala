...
sealed trait MyOption[+A] {
  ...
  /**
    ...
    * @return 新しい [[MyOption]]
    */
  def map[B](f: A => B): MyOption[B] = if (isEmpty) MyNone else MySome[B](f(get))
  ...
  /**
    ...
    * @return 新しい [[MyOption]]
    */
  def flatMap[B](f: A => MyOption[B]): MyOption[B] = if (isEmpty) MyNone else f(get)
  ...
  def filter(f: A => Boolean): MyOption[A] = {
    ...
    if (f(get)) MySome[A](get) else MyNone
  }
